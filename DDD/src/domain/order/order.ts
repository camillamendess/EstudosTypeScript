...
export class Order extends Aggregate {
  ...
  addItem(item: Item) {
    const itemAlreadyExists = this._items.find((i) => i._id.equals(item._id));
  ...
  updateItemName(id: Id, name: string) {
    const itemIndex = this._items.findIndex((i) => i._id.equals(id));
    if (itemIndex === -1) {
      throw new Error("Item not found");
    }

    this._items[itemIndex]._name = name;
  }
